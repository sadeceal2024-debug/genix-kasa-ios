...
class ViewController: UIViewController, WKNavigationDelegate, UIDocumentInteractionControllerDelegate {
    ...
    override func viewDidLayoutSubviews() {
        ...
        KasaTakip.webView.frame = calcWebviewFrame(webviewView: webviewView, toolbarView: nil)
    }
    
    @objc func keyboardWillHide(_ notification: NSNotification) {
        KasaTakip.webView.setNeedsLayout()
    }
    
    func initWebView() {
        KasaTakip.webView = createWebView(container: webviewView, WKSMH: self, WKND: self, NSO: self, VC: self)
        webviewView.addSubview(KasaTakip.webView);
        
        KasaTakip.webView.uiDelegate = self;
        
        KasaTakip.webView.addObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress), options: .new, context: nil)

        if(pullToRefresh){
            let refreshControl = UIRefreshControl()
            refreshControl.addTarget(self, action: #selector(refreshWebView(_:)), for: UIControl.Event.valueChanged)
            KasaTakip.webView.scrollView.addSubview(refreshControl)
            KasaTakip.webView.scrollView.bounces = true
        }

        if #available(iOS 15.0, *), adaptiveUIStyle {
            themeObservation = KasaTakip.webView.observe(\.themeColor) { [unowned self] webView, _ in
                let backgroundColor = KasaTakip.webView.underPageBackgroundColor;
                let themeColor = KasaTakip.webView.themeColor;
...
//        toolbarView.autoresizingMask = [.flexibleTopMargin, .flexibleRightMargin, .flexibleWidth]
    ...
    func overrideUIStyle(toDefault: Bool = false) {
        if #available(iOS 15.0, *), adaptiveUIStyle {
            if (((htmlIsLoaded && !KasaTakip.webView.isHidden) || toDefault) && self.currentWebViewTheme != .unspecified) {
                UIApplication
                    .shared
                    .connectedScenes
    ...
    @objc func loadRootUrl(cachePolicy: NSURLRequest.CachePolicy = .useProtocolCachePolicy) {
        KasaTakip.webView.load(URLRequest(url: SceneDelegate.universalLinkToLaunch ?? SceneDelegate.shortcutLinkToLaunch ?? rootUrl, cachePolicy: cachePolicy))
    }
    ...
    func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!){
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.8) {
            KasaTakip.webView.isHidden = false
    ...
    override func observeValue(forKeyPath keyPath: String?, of object: Any?, change: [NSKeyValueChangeKey : Any]?, context: UnsafeMutableRawPointer?) {
        ...
        if (keyPath == #keyPath(WKWebView.estimatedProgress) &&
                KasaTakip.webView.isLoading &&
                !self.loadingView.isHidden &&
                !self.htmlIsLoaded) {
                    var progress = Float(KasaTakip.webView.estimatedProgress);
    ...
    deinit {
        KasaTakip.webView.removeObserver(self, forKeyPath: #keyPath(WKWebView.estimatedProgress))
    }
}
...
extension ViewController: WKScriptMessageHandler {
  func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
        if message.name == "print" {
            printView(webView: KasaTakip.webView)
        }
...
extension ViewController {
    ...
    func sendSpeechToJS(_ text: String, _ isFinal: Bool, _ error: String?) {
        ...
        DispatchQueue.main.async {
            KasaTakip.webView?.evaluateJavaScript(js, completionHandler: nil)
        }
    }
}
...
extension ViewController {
    func handleIAP(_ message: WKScriptMessage) {
        guard #available(iOS 15.0, *) else {
            KasaTakip.webView?.evaluateJavaScript(
        ...
        let productId = (body["productId"] as? String) ?? "com.genixsoft.kasatakip.pro.yearly"
        switch action {
